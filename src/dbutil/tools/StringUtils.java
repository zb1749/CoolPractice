...
public final class StringUtils extends org.apache.commons.lang.StringUtils {
     ...
     * 获得文本长度，其中双字节字符按2计算。
     * 举例：
     * getLengthInBytes("中国")  = 4
     * getLengthInBytes("卡拉OK")  = 6
     * getLengthInBytes("太阳　月亮")  = 10
     *
     * @param str
     * @return
     */
    ...
    //
    // /**
    // * 将文字用当前默认地区编码解码，然后按指定的编码重新解码String
    // *
    // * @param str
    // * @param encode
    // * @return
    // */
    // public static String fixCharset(String str, String errCharset, String
    // properCharset) {
    // try {
    // return new String(str.getBytes(errCharset), properCharset);
    // } catch (UnsupportedEncodingException e) {
    // throw new RuntimeException(e);
    // }
    // }
    ...
    public static final boolean toBoolean(String s, Boolean defaultValue) {
        if (StringUtils.isEmpty(s))//空提前返回
            return defaultValue;
     ...
     * HTML转义
     */
    public static String escapeHtml(CharSequence s, boolean unicode) {
        if (unicode)
    ...
    public static String insert(String source, int n, String str) {
        if (source.length() <= n) return source.concat(str);
        if (n < 0) n = 0;
     ...
     * @return
     */
    public static Map<String, String> toMap(String source, String entrySep, String keyValueSep) {
        ...
        if (source != null) {
            for (String entry : StringUtils.split(source, entrySep)) {
                ...
                if (index > -1) {
                    result.put(entry.substring(0, index).trim(), entry.substring(index + 1).trim());
                } else {
                    result.put(entry, "");
                }
            }
     ...
     * @param entrySep           entry间的分隔符
     * @param keyValueSep        key/value的分隔符
     * @param useUppercaseForKey 是否要将KEY值转成大写
     * @return
     */
    public static Map<String, String> toMap(String source, String entrySep, String keyValueSep,
                                            boolean useUppercaseForKey) {
        Map<String, String> result = new LinkedHashMap<String, String>();
        if (source != null) {
            for (String entry : StringUtils.split(source, entrySep)) {
                entry = entry.trim();
                int index = entry.indexOf(keyValueSep);
                if (index > -1) {
                    result.put(useUppercaseForKey ?
                                    entry.substring(0, index).trim().toUpperCase() :
                                    entry.substring(0, index).trim(),
                            entry.substring(index + 1).trim());
                } else {
                    result.put(useUppercaseForKey ? entry.toUpperCase() : entry, "");
                }
            }
        }
        return result;
